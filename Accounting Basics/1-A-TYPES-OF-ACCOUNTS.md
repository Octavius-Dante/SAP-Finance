## Real Account
These accounts relate to the assets and properties of a business, including tangible items (cash, machinery, furniture) and intangible items (goodwill, patents). They represent resources owned by the business that carry over from year to year. 

**Rule: Debit what comes in, Credit what goes out.** </br>
Examples: Cash A/c, Land & Building A/c, Machinery A/c, Inventory A/c. 

## Personal Account
These accounts represent specific persons, companies, firms, or organizations with whom the business deals. They track debts, receivables, and payables. 

**Rule: Debit the receiver, Credit the giver.** </br>
Examples: Ram’s A/c, XYZ Ltd, State Bank of India, Creditors A/c, Debtors A/c. 

## Nominal Account
These accounts are related to expenses, losses, income, and gains. These are temporary accounts that do not exist physically and are zeroed out at the end of the financial year to calculate net profit or loss. 

**Rule: Debit all expenses and losses, Credit all incomes and gains.** </br>
Examples: Salary A/c, Rent Expense A/c, Interest Received A/c, Commission Earned

</br></br>

## Difference Summary 

<table>
    <tr>
        <td>Feature </td>
        <td>Real Account</td>
        <td>Personal Account</td>
        <td>Nominal Account</td>
    </tr>
    <tr>
        <td>Nature</td>
        <td>Assets/Property</td>
        <td>Individuals/Entities</td>
        <td>Income/Expense</td>
    </tr>
    <tr>
        <td>Rule</td>
        <td>Debit in, Credit out</td>
        <td>Debit receiver, Credit giver</td>
        <td>Debit expense, Credit gain</td>
    </tr>
    <tr>
        <td>Balance</td>
        <td>Carried forward</td>
        <td>Carried forward</td>
        <td>Closed at year-end</td>
    </tr>
    <tr>
        <td>Example</td>
        <td>Furniture, Cash</td>
        <td>Bank, Vendor</td>
        <td>Salary, Interest</td>
    </tr>
</table>
